import turtle as t;t.width(2);t.speed(0);t.ht();#Importing turtle, setting the settings up correctly - defining resolution so screen will scale. Ignore the comprehensions... they work, trust me.
...
def draw(a):#Defining functions to draw bonds, double bonds or atoms
    global t, res
    t.clearscreen()
    ...
    if info[0] == "alkane":runtime = ["h"]+[["h","c","h"] for x in range(info[1])]+["h"]
        ...
        err = "Unknown/unsupported molecule: " + a
...
if __name__ == '__main__':
    while True:
        try:
            draw(input("What would you like to be drawn:\n"))
            print("Done")
        except:
            print("The script encountered a fatal error. Please check the input.")
